    ...
    <table class="mt-5 table">
        ...
        <tbody>
            @forelse ($users as $user)
                <tr wire:key ="{{ $user->id }}" class=" hover:bg-blue-50 duration-150 ease-in-out transition-all">

                    <td class="px-2 py-2">
                        <img src="{{ asset($user->image) ?? '1.jpg' }}" alt="profile image"
                    ...
                    @php
                        if ($user->payment_status == 'paid') {
                            $textColor = 'badge text-green-500 px-4 py-3 bg-transparent border-none';
                        } else {
                            $textColor = 'badge text-red-500 px-4 py-3 bg-transparent border-none';
                        }
                    @endphp

                    <td class="px-2 py-2"> {{ $user->name }} </td>
                    <td class="px-2 py-2"> {{ $user->email }} </td>
                    <td class="px-2 py-2"> {{ $user->nic }} </td>
                    <td class="px-2 py-2"> {{ $user->address }} </td>
                    <td class="px-2 py-2"> {{ $user->phone_no }} </td>
                    <td class="px-2 py-2"> {{ $user->weight }} Kg </td>
                    <td class="px-2 py-2"> {{ $user->height }} cm </td>
                    <td class="px-2 py-2">
                        <div class="{{ $textColor }} w-full flex justify-center">
                    ...
                    <td class="px-2 py-4 h-full flex gap-2 items-center">
                        ...
                        <a wire:navigate href="{{ route('user.show', $user->id) }}"
                            class="text-yellow-600 hover:text-green-500 transition-all ease-in-out duration-150 underline mr-2">
                            view </a>
                ...
                <tr>
                    <td colspan="9">
                        <div class="flex flex-col gap-3 py-8 justify-center items-center">
                            <svg class="w-12 h-12 text-yellow-400" xmlns="http://www.w3.org/2000/svg"
                            ...
                            <h1 class="text-lg font-bold"> No members found ! </h1>
                        </div>
                    </td>
                </tr>
            @endforelse
