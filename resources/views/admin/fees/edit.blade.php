<x-admin-layout>
    <div class="px-[8%] py-[1%] flex flex-col">
        ...
        {{-- Create user form --}}
        <form action="{{ route('fees.update', $fee->id) }}" method="POST" class="mt-5">
            @csrf
            @method('PATCH')
            <div class="grid grid-cols-3 gap-6">
                    ...
                    <label class="form-control w-full">
                        ...
                        <select class="select select-bordered" name='user_id' id='user_id'>
                            ...
                            @foreach ($members as $key => $member)
                                <option value="{{ $key }}" {{ $fee->user_id == $key ? 'selected' : '' }}>
                                    {{ $member }} </option>
                            @endforeach
                        </select>
                    </label>
                ...
                <div>
                    <label class="form-control w-full">
                        ...
                        <input type="date" name="date" id="date" value="{{ $fee->date }}"
                ...
                <div>
                    <label class="form-control w-full">
                        ...
                        <input type="text" name="amount" id="amount" value="{{ $fee->amount }}"
                            class="input input-bordered w-full" />
                    </label>
                </div>
            </div>
